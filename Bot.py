...
class Action:
    ...
    def getUser(self):
        return self.user
class Clear:
    def __init__(self, planelist, pool):
...
print(app_access)
url = 'https://api.twitch.tv/helix/streams?'          #url for getting viewer count
...
params = {'user_login':channel.replace("#","")}
...
while True:
    ...
    if timerclass.time() > capreqtimer:
        sock.send("CAP REQ :twitch.tv/tags\r\n".encode('utf-8'))
        sock.send("CAP REQ :twitch.tv/commands\r\n".encode('utf-8'))
        capreqtimer += 60
    ...
    if "--" not in message and "—" not in message:              #logic gate to prevent more CPU usage if command not present
        ...
        if len(usercount) != 0:
            ...
            socksend("Tracking is turned off\r\n")
        continue
        ...
        if user in authorized:
            person = re.findall("restore (.+)", message)
            person = str(person)
            person = person[2:len(person)-2]
            print(person)
            print(users)
            if person in users:
        ...
        if user in authorized:
            person = re.findall("revoke (.+)", message)
            person = str(person)
            person = person[2:len(person)-2]
            if person in usercount.keys():
                ...
                socksend(f"No probable cause for depleting {person}\r\n")
        continue
            ...
            if highlighted == "highlighted-message" or highlighted == "0f4fa868-f365-44a7-9fe9-2d793556fb50":
                ...
                if result == "No match" or result == "Bombers are useless":
                    ...
                    if index == -1:
                        ...
                        pool.insert(random.randrange(len(pool)+1), newplane)
    ...
    elif "--pick" in message or "—pick" in message:
        if user in authorized:
            if len(pool) != 0:
                integer = random.randint(0, (len(pool)-1))
                socksend(f"{pool[integer]} has been chosen by the gods\r\n")
            else:
                socksend("Request pool is empty\r\n")
        continue
